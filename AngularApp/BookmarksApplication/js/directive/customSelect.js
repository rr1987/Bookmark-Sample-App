(function (angular, undefined) {
	...
	var module = angular.module('customDirective', []);

	module.value('customSelectDefaults', {
		displayText: 'Select...',
		emptyListText: 'There are no folders to display',
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					'<div class="dropdown-menu">' +
						...
						'<div class="custom-select-action">' +
							...
							'<button type="button" class="btn btn-primary btn-block add-button" ng-click="add(searchTerm)">{{addText}}</button>' +
						'</div>' +
					'</div>';
				...
				function configChildScope() {
					...
					childScope.add = function (searchTerm) {
						$q.when(options.onAdd(searchTerm), function (item) {
							...
							childScope.matches.push(searchTerm);
							childScope.select(searchTerm);
						});
					};
